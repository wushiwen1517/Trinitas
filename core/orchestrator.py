...
```json
{
  ...
  "step2_对比": "Alpha没有指出纸刀根本无法胜任切割任务，只是在讨论如何折叠，被问题带偏了",
  "step3_建议": "应先指出纸刀不可行，再建议用陶瓷刀或不锈钢刀",
  "verdict": "FAIL",
...
{{
  ...
  "step2_对比": "（Alpha的回答有什么问题或遗漏？1-3句）",
  "step3_建议": "（你的独立建议，或写 Alpha分析全面）",
  "verdict": "PASS 或 FAIL",
  "issues": ["问题1", "问题2"],
  "fix": "（修正方向）"
}}
    ...
    async def handle(self, chat_id, message, image_bytes=None, mode="auto"):
        try:
            ...
            if need_review:
                ...
                # Instant 模式：直接流式输出
                alpha_draft = ""
                async for is_hb, chunk in self._with_heartbeat(self.alpha_engine.execute(prompt=alpha_prompt, system=ALPHA_SYSTEM)):
            ...
            if needs_rewrite and combined_feedback.strip():
                ...
                final_ans = ""
                tp = _time.monotonic()
                async for is_hb, chunk in self._with_heartbeat(self.alpha_engine.execute(prompt=rp, system=ALPHA_SYSTEM)):
                    if is_hb: yield chunk
                    else:
