...
BETA_SYSTEM = """你是 Trinitas-Beta，一个独立的逻辑审查员。你的任务是审查回答，找出错误和盲区。
你必须用中文输出。你必须严格按 JSON 格式输出审查结果。

关键要求：
- 你必须同时审题（用户问题本身）与审答（Alpha回答）。
- issues 只能写“真实问题”（能明确指出为什么错/缺失）；如果确实没有错误，issues 可以为空数组。
- 但 improvements 必须至少 1 条（可以是：更清晰的定义、更好的步骤、更严谨的边界条件、更安全的默认值、更友好的排版等），避免为了凑数而编造错误。"""

GAMMA_SYSTEM = """你是 Trinitas-Gamma，最终裁判。你的任务是综合评估回答质量并给出裁决。
你必须用中文输出。你必须严格按 JSON 格式输出裁决结果。

关键要求：
- 站在“执行者/落地者”的视角判断：照做是否会成功？会不会踩坑？
- issues 只能写真实问题；如果没有硬伤可为空数组。
- improvements 必须至少 1 条（可写：补充边界、风险提示、测试/验证步骤、替代方案、排版可读性等）。"""
...
```json
{
  "step0_审题陷阱": "问题隐含前提错误：材料强度不足，目标不可达",
  "step1_关键缺失": "Alpha未先否定前提，导致方案看似可行但实际上不可执行",
  "step2_独立方案": "先解释不可行原因，再给可行替代（陶瓷/不锈钢刀）与注意事项",
  "step3_可执行修复": "补充材料强度与切割机理，明确‘折叠增硬’只能切软物、无法切牛排",
  "verdict": "FAIL",
  "issues": ["纸的抗拉强度不足以切割肉类纤维", "未指出方案的根本不可行性"],
  "improvements": ["输出结构可用‘结论→原因→替代方案’提升可读性"],
  "fix": "应先否定前提，再给替代方案"
}
```
...
```json
{
  ...
  "improvements": ["补充‘测试计划/验收标准’，让执行者知道做到什么算完成"],
...
```json
{{
  "step0_审题陷阱": "（问题本身是否有陷阱/缺条件/伪命题？没有则写无）",
  "step1_关键缺失": "（Alpha回答的缺失/错误/误导点，1-4句）",
  "step2_独立方案": "（跳出 Alpha 思路，给更优/更稳的解法，1-4句）",
  "step3_可执行修复": "（给 Alpha 的可执行修正要点，尽量具体）",
  "verdict": "PASS 或 FAIL",
  "issues": ["问题1", "问题2"],
  "improvements": ["改进点1（即使 PASS 也至少 1 条）"],
  "fix": "（修正方向）"
}}
```"""
...
```json
{{
  "step0_复述目标": "（把用户想要的结果复述成可验收的目标，1句）",
  "step1_可执行性": "（照做能否成功？缺了什么？1-3句）",
  "step2_风险与边界": "（最坏情况/边界条件/依赖前提，1-4句）",
  "step3_最终建议": "（给最终落地建议/替代路径/验证步骤，1-4句）",
  "verdict": "PASS 或 FAIL",
  "confidence": 0.0到1.0,
  "issues": ["问题1"],
  "improvements": ["改进点1（即使 PASS 也至少 1 条）"],
...
请根据反馈重新生成修正后的完整答案。直接给出答案，不解释修改了什么。"""
    ...
    async def _collect_with_heartbeat(self, engine, prompt, all_stats, system=None):
        ...
        buf = ""
        # 注意：部分网络/隧道/代理对“过小分片”会缓冲，导致前端读流超时。
        # 这里使用更高频率 + 更大心跳分片，尽量确保浏览器端能及时收到字节。
        async for is_hb, chunk in self._with_heartbeat(
            engine.execute(prompt=prompt, system=system),
            interval=5.0,
            hb=("\u200b" * 64),
        ):
            if is_hb:
    ...
    async def handle(self, chat_id, message, image_bytes=None, mode="auto"):
        try:
            ...
            # 构建上下文可能触发历史压缩（会调用模型，耗时较长）。
            # 为避免前端“stream idle timeout”，这里也透传心跳。
            yield self._phase("CONTEXT_BUILD")
            tp_ctx = _time.monotonic()
            ctx_task = asyncio.create_task(self._build_context(chat_id))
            while not ctx_task.done():
                done, _ = await asyncio.wait([ctx_task], timeout=5.0)
                if not done:
                    yield ("\u200b" * 64)
            context_text = ctx_task.result()
            timings["CONTEXT"] = round(_time.monotonic() - tp_ctx, 2)
            ...
            if need_review:
                ...
                alpha_draft = ""
                async for is_hb, chunk in self._with_heartbeat(
                    self.alpha_engine.execute(prompt=alpha_prompt, system=ALPHA_SYSTEM),
                    interval=5.0,
                    hb=("\u200b" * 64),
                ):
            ...
            if needs_rewrite and combined_feedback.strip():
                ...
                tp = _time.monotonic()
                async for is_hb, chunk in self._with_heartbeat(
                    self.alpha_engine.execute(prompt=rp, system=ALPHA_SYSTEM),
                    interval=5.0,
                    hb=("\u200b" * 64),
                ):
                    if is_hb: yield chunk
